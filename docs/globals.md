[**@zenstone/ts-utils**](README.md)

***

# @zenstone/ts-utils

## Type Aliases

- [AryHeaderItem](type-aliases/AryHeaderItem.md)
- [DirectorySeparator](type-aliases/DirectorySeparator.md)
- [ErrLike](type-aliases/ErrLike.md)
- [HeadersInitInput](type-aliases/HeadersInitInput.md)
- [HttpStatusCode](type-aliases/HttpStatusCode.md)
- [PathInput](type-aliases/PathInput.md)
- [PathReplacementCallback](type-aliases/PathReplacementCallback.md)
- [PathUtilsOptions](type-aliases/PathUtilsOptions.md)
- [RecordObj](type-aliases/RecordObj.md)
- [ResponseInitInput](type-aliases/ResponseInitInput.md)

## Variables

- [UnixDS](variables/UnixDS.md)
- [WinDS](variables/WinDS.md)

## Functions

- [calcProgress](functions/calcProgress.md)
- [ceil10](functions/ceil10.md)
- [createPathUtils](functions/createPathUtils.md)
- [decimalAdjust](functions/decimalAdjust.md)
- [errMsg](functions/errMsg.md)
- [floor10](functions/floor10.md)
- [isErrLike](functions/isErrLike.md)
- [isInferObj](functions/isInferObj.md)
- [isNumber](functions/isNumber.md)
- [isNumberVal](functions/isNumberVal.md)
- [isStr](functions/isStr.md)
- [joinHttpPath](functions/joinHttpPath.md)
- [limitNumberMax](functions/limitNumberMax.md)
- [limitNumberMin](functions/limitNumberMin.md)
- [limitNumberMinMax](functions/limitNumberMinMax.md)
- [mergeAbortSignals](functions/mergeAbortSignals.md)
- [mergeHeaders](functions/mergeHeaders.md)
- [mergeRespInit](functions/mergeRespInit.md)
- [notEmptyStr](functions/notEmptyStr.md)
- [purgeHttpPath](functions/purgeHttpPath.md)
- [round10](functions/round10.md)
- [toAryHeaders](functions/toAryHeaders.md)
- [toNumber](functions/toNumber.md)
