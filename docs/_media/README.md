[**@zenstone/ts-utils**](../README.md)

***

[@zenstone/ts-utils](../modules.md) / async

# async

## Classes

- [PendingScopeConflictError](classes/PendingScopeConflictError.md)
- [RetryExhaustedError](classes/RetryExhaustedError.md)
- [TimeoutError](classes/TimeoutError.md)

## Type Aliases

- [AnyFn](type-aliases/AnyFn.md)
- [PendingCallbackFn](type-aliases/PendingCallbackFn.md)
- [PendingFnParams](type-aliases/PendingFnParams.md)
- [RetryCallbackFn](type-aliases/RetryCallbackFn.md)
- [RetryFnParams](type-aliases/RetryFnParams.md)
- [RetryOptions](type-aliases/RetryOptions.md)

## Functions

- [clearPendingRegistry](functions/clearPendingRegistry.md)
- [pending](functions/pending.md)
- [pendingFn](functions/pendingFn.md)
- [retry](functions/retry.md)
- [retryFn](functions/retryFn.md)
- [sleep](functions/sleep.md)
- [timeout](functions/timeout.md)
