...
import { RpcAbortError, StatefulRpc, type StatefulRpcTask } from './StatefulRpc';
...
describe('StatefulRpc - timeout', () => {
  ...
  it('abort clears the timer — no late TimeoutError', async () => {
    ...
    const rpc = new StatefulRpc<number, string>({
      timeout: 30,
      events: { reject: (ev) => { if (ev.result instanceof TimeoutError) timedOut = true; } },
    });
    const p = rpc.pending('r', 'd');
    rpc.abort('r');
    const err = await p.catch((e) => e);
...
describe('StatefulRpc - events', () => {
  ...
  it('resolve event fires with task and result', async () => {
    ...
    const rpc = new StatefulRpc<number, string>({
      events: {
        resolve: ({ task, result }) => void calls.push({ key: task.key, result }),
      },
    });
    const p = rpc.pending('r', 'd');
    rpc.resolve('r', 42);
